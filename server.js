...
app.get('/api/items', (req, res) => {
  res.json({message: `Book List`, books});
});
...
app.get('/api/items/search', (req, res) => {
  ...
  const bookList = req.query.title.toLowerCase();
  const searchBook = books.filter(book => book.toLocaleLowerCase().includes(bookList));
  res.json({message: `Book Title found: ` + searchBook});
});
...
app.get('/api/items/:id', (req, res) => {
  ...
  res.json({message:`Book Title found: ` + books[id]})
});
...
app.post('/api/items', (req, res) => {
  ...
  res.json({message: `Book was added to list: ${title}` , books});
...
app.put('/api/items/:id', (req, res) => {
    ...
    //fix this later
    res.json({message: `${title} was successfully added to the list.`, titles: books});
});
...
app.delete('/api/items/:id', (req, res) => {
  ...
  const removeTitle = books.splice(id,1);

  res.json({message: `${removeTitle} was removed from list.`, books});
